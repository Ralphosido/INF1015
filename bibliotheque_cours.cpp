...
static bool displayDebugString(const std::string& message)
{
	...
	if constexpr (executionEstUtf8) {
		...
		resultat.resize(MultiByteToWideChar(CP_UTF8, 0, &message.front(), message.size(), &resultat.front(), resultat.size()));
...
/*
template <auto fonctionRemplace>
class RemplaceRemet {
public:
	template <typename T>
	RemplaceRemet(T&& nouvelleValeur)
		: ancienneValeur(fonctionRemplace(std::forward<T>(nouvelleValeur)))
	{}
	~RemplaceRemet() {
		(void)fonctionRemplace(std::move(ancienneValeur))
	}
...
private:
	invoke_result_t<fonctionRemplace> ancienneValeur;
};
*/
